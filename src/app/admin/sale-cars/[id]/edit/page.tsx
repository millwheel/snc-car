...
export default function SaleCarEditPage() {
  const { id } = useParams();
  const router = useRouter();
  const [item, setItem] = useState<SaleCarRow | null>(null);
  ...
  if (notFound || !item) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        ...
        <button
          onClick={() => router.push('/admin/sale-cars')}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="bg-bg-card rounded-xl shadow-sm p-6">
        <SaleCarForm
          saleCar={item}
          onSuccess={() => router.push(`/admin/sale-cars/${id}`)}
