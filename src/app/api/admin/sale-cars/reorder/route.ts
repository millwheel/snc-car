...
export async function PATCH(request: Request) {
  ...
  const { saleCarId, direction } = body as {
    saleCarId: number;
    direction: 'up' | 'down';
  };

  if (!saleCarId || !['up', 'down'].includes(direction)) {
    return NextResponse.json(
      { error: 'saleCarId와 direction(up/down)이 필요합니다' },
      { status: 400 }
    );
  }
  ...
  const { error: updateError1 } = await supabase
    ...
    .eq('sale_car_id', current.sale_car_id);
  ...
  const { error: updateError2 } = await supabase
    ...
    .eq('sale_car_id', adjacentItem.sale_car_id);
